...
export function buildPresetMaterial(id) {
  switch (id) {
    case 'solid': { const hsl = state.presetParams['solid-color-hsl'], rough = gp('solid', 'rough', 20); const mat = new THREE.MeshPhysicalMaterial({ metalness: 0.0, roughness: rough / 100, clearcoat: 0.6, clearcoatRoughness: 0.1, envMapIntensity: 1.2 }); if (hsl) mat.color.setHSL(hsl[0] / 360, hsl[1], hsl[2]); else { const col = gp('solid', 'color', 230); mat.color.setHSL(col / 360, 0.5, 0.65); } return mat; }
